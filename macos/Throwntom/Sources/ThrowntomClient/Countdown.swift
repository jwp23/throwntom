...
public enum Countdown {
    ...
    private static let clock = try! NSRegularExpression(pattern: #"\d{2,}:\d{2}"#)
    ...
    public static func tickedStatusLine(_ state: DaemonState, now: Date) -> String {
        ...
        guard let match = clock.firstMatch(in: line, range: NSRange(line.startIndex..., in: line)),
              let range = Range(match.range, in: line) else { return line }
        return line.replacingCharacters(in: range, with: formatRemaining(end.timeIntervalSince(now)))
    }
