...
public struct SSEFrameSplitter {
  ...
  private func frameEnd() -> (frameEnd: Data.Index, nextStart: Data.Index)? {
    ...
    switch (lf, crlf) {
    case (let l?, let c?): return l.lowerBound <= c.lowerBound ? (l.lowerBound, l.upperBound) : (c.lowerBound, c.upperBound)
    case (let l?, nil): return (l.lowerBound, l.upperBound)
    case (nil, let c?): return (c.lowerBound, c.upperBound)
