...
final class CountdownTests: XCTestCase {
    private func state(_ phase: DaemonState.Phase, endsIn seconds: TimeInterval?, line: String, now: Date) -> DaemonState {
        DaemonState(state: phase, phaseEndAt: seconds.map { now.addingTimeInterval($0) }, pausedRemaining: 0,
              completedToday: 3, workSessionsInBlock: 1, longBreakEvery: 4, nextStage: nil,
              morningPending: false, snoozeUntil: nil, statusLine: line, focusedTaskIds: [])
    }
    ...
    func testLeavesIdlePausedAndConfirmLinesAlone() {
        let now = Date()
        for (phase, line) in [(DaemonState.Phase.idle, "Idle  Today: 0  Cycle: 0/4"),
