...
final class ConnectionStatusTests: XCTestCase {
    ...
    func testPlaceholderTextNilWhenStatePresent() {
        let state = DaemonState(
            state: .idle, phaseEndAt: nil, pausedRemaining: 0, completedToday: 0,
            workSessionsInBlock: 0, longBreakEvery: 4, nextStage: nil, morningPending: false,
            snoozeUntil: nil, statusLine: "Idle", focusedTaskIds: [])
        XCTAssertNil(ConnectionStatus.placeholderText(state: state, connection: .connected, now: .now))
    }

    func testPlaceholderTextConnecting() {
        XCTAssertEqual(
            ConnectionStatus.placeholderText(state: nil, connection: .connecting, now: .now), "Throwntom…")
    }

    func testPlaceholderTextStartingDaemon() {
        XCTAssertEqual(
            ConnectionStatus.placeholderText(state: nil, connection: .startingDaemon, now: .now), "Starting timer…")
    }
    ...
    func testPlaceholderTextConnected() {
        XCTAssertEqual(
            ConnectionStatus.placeholderText(state: nil, connection: .connected, now: .now), "Throwntom")
    }
}
