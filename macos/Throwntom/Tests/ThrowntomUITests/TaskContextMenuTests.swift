...
final class TaskContextMenuTests: XCTestCase {
  ...
  func testChoosingAnItemActsOnThatRowNotTheSelection() async throws {
    ...
    let menu = TaskContextMenu(task: makeTask(id: 8), client: environment.client, model: environment.model)
    menu.run(.complete)
  ...
  func testNewTaskFromTheMenuOpensTheEditor() throws {
    ...
    let menu = TaskContextMenu(task: makeTask(id: 1), client: environment.client, model: environment.model)
    menu.run(.newTask)
