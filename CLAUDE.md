...
- `macos/` — SwiftUI window app (Swift package), bundle resources, build.sh/agent.sh
...
- Swift style: `macos/swift-lint.sh` (Airbnb SwiftFormat + SwiftLint, pinned versions; `--fix` autocorrects). Runs in pre-commit when Swift files are staged and in CI. If the installed tool doesn't match the pin, the script downloads and checksum-verifies the pinned release itself and caches it under `macos/.swift-lint-cache` — no manual PATH juggling needed.
- Design: read `DESIGN.md` before UI, styling, or component work (TUI or macOS); after editing it, run `npx @google/design.md@0.4.0 lint DESIGN.md`.
...
- Worktrees live inside the repo, not beside it. A sibling `../throwntom-<description>`
  ...
  gone. `.claude/worktrees/` is gitignored, so `git status` won't surface a stale one
  there either — use `git worktree list` or a filesystem check to find them.
...
- Put the citations in the PR description, under a `Doc claims` heading; when the
  work has no PR, put them in the final response. One line per claim: the
  sentence, and the `file:line` that proves it.
...
- A diff that changes no prose writes "no behavioural claims made" in that same
  place and is done.
