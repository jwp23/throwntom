...
- `tools/swiftmutantsgate/` — fails on any unexcluded Survived, Crash, Timeout or NoCoverage mutant in swift-mutation-testing JSON reports and counts Unviable ones; `-summary` writes the per-file table the weekly Swift mutation workflow files as its tracking issue
