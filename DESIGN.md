...
Every affordance that reaches the daemon is withdrawn on all three: the timer chips, the Timer menu and its key equivalents, the whole Tasks menu and the row context menu, and the Tasks and Stats chips and View-menu items. Local commands stay live — Keyboard Shortcuts, Open Config File…, the login-item and settings items, and Start Timer Service itself, the one control whose whole point is to work when nothing else does. The cheat sheet stays the complete list of what the app binds, because it is a reference and dispatches nothing.
