...
A running snooze also prints `Snoozed · MM:SS left` under the chips, in body text like the window's other status lines rather than as a caption: snoozing withdraws the reminder banner, so that line is the whole of what an active snooze has on screen to show for it, and it is what the user reads to find out when the reminder comes back.
