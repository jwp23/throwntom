...
**Mascot.** The `mascot-*` tokens are the character's own paint and never appear elsewhere: `mascot-body-light` → `mascot-body` → `mascot-body-dark` is the body's radial gradient (centre upper-left, dark at the rim); `mascot-leaf-light` → `mascot-leaf-dark` the leaf gradient; `mascot-blush` the cheeks; `mascot-prop-light`/`-dark` the laptop; `mascot-wood` the book; `mascot-sky` the drink, screen and yo-yo. The sofa is painted in the phase's ground, `*-panel` and a mid-tone between them so it belongs to the room it is in. Hands are `mascot-body`; every outline is `macos-outline`.
